...
<td>ID:  <%=internshipCoordinator.getId()%></td>
</tr>
...
<input required type="text" name="code" value="<%=internshipCoordinator.getCode()%>" size="30">
</td>

<td>
VALUE<font color="red">*</font>:<br/>
<input required type="text" name="value" value="<%=internshipCoordinator.getValue()%>" size="30">
</td>
...
<td>
SHORTNAME<font color="red">*</font>:<br/>

<input required type="text" name="shortName" value="<%=internshipCoordinator.getShortName()%>" size="30">
</td>
...
<input type="hidden" name="page" value= "saveShowNextInternshipCoordinator">
<input type="hidden" name="pageNo" value= "<%=pageNo%>">
<input type="hidden" name="limit" value= "<%=limit%>">
<input type="hidden" name="id" value= "<%=internshipCoordinator.getId()%>">
</form>

<form action="internshipCoordinatorCntrl"  id="myform1" name="myform1" accept-charset="UTF-8" >
<input type="hidden" name="page" value= "saveShowNextInternshipCoordinator">
<input type="hidden" name="pageNo" value= "<%=pageNo%>">
<input type="hidden" name="limit" value= "<%=limit%>">
<input type="hidden" name="id" value= "<%=internshipCoordinator.getId()%>">
...
<% dropDownDBService.closeConnection(); %>
    ...
    <% Iterator<InternshipCoordinator> it1= internshipCoordinatorList.iterator(); %>
    
<%if(pageNum!=0){ %>   	
	<div align="left">	
	<font color="blue" size="3">	
	<%if(pageNum==1){%>|&lt;&nbsp;&nbsp;&nbsp;&lt;&lt;<%} else { %><a href="internshipCoordinatorCntrl?page=internshipCoordinatorDashboard&opr=showAll&pageNo=<%=1%>&limit=<%=limit%>">|&lt;</a>&nbsp;&nbsp;&nbsp;<a href="internshipCoordinatorCntrl?page=internshipCoordinatorDashboard&opr=showAll&pageNo=<%=pageNum-1%>&limit=<%=limit%>">&lt;&lt;</a><%}%>	
	&nbsp;&nbsp;&nbsp;Page: <%=pageNum%>/<%=totalPages%>	
	<%if(pageNum==totalPages){%>&nbsp;&nbsp;&nbsp;&gt;&gt;&nbsp;&nbsp;&nbsp;&gt;|<%} else { %>&nbsp;&nbsp;&nbsp;<a href="internshipCoordinatorCntrl?page=internshipCoordinatorDashboard&opr=showAll&pageNo=<%=pageNo+1%>&limit=<%=limit%>">&gt;&gt;</a>&nbsp;&nbsp;&nbsp;<a href="internshipCoordinatorCntrl?page=internshipCoordinatorDashboard&opr=showAll&pageNo=<%=totalPages%>&limit=<%=limit%>">&gt;|</a><%}%>	
	</font>	
	</div>    	
	<%} %>	
...
<% while(it1.hasNext()){
	InternshipCoordinator internshipCoordinator1 = (InternshipCoordinator)it1.next();
	%>
<tr>
<td><%=count++%></td>
<td><%=internshipCoordinator1.getId()%></td>
<td><%=internshipCoordinator1.getCode()%></td>
<td><%=internshipCoordinator1.getValue()%></td>
<td><%=internshipCoordinator1.getShortName()%></td>
<td><a href="internshipCoordinatorCntrl?page=saveShowNextInternshipCoordinator&opr=edit&id=<%=internshipCoordinator1.getId()%>&pageNo=<%=pageNo%>&limit=<%=limit%>"><button type="button" class="btn">Edit</button></a></td>
<td><a href="internshipCoordinatorCntrl?page=saveShowNextInternshipCoordinator&opr=delete&id=<%=internshipCoordinator1.getId()%>&pageNo=<%=pageNo%>&limit=<%=limit%>"><button type="button" class="btn1">Delete</button></a></td>
</tr>
	<%} %>
</tbody>
	</table>    </div>
